...
sealed trait ValidKey[T] extends (T => js.Any) { def apply(x:T): js.Any = x.asInstanceOf[js.Any] }
object ValidKey {
  ...
  implicit object IntArrayOk extends ValidKey[js.Array[Int]]
  implicit object DoubleArrayOk extends ValidKey[js.Array[Double]]
  implicit object StringArrayOk extends ValidKey[js.Array[String]]
  implicit object JsDateOk extends ValidKey[js.Date]
}

abstract class IdbSupport[K, V](
  ...
  object StoreKeyPolicy {
    implicit object implicitly extends StoreKeyPolicy[V] {
      ...
      def add(input: V, store: IDBObjectStore): IDBRequest = {
        println(s"====> adding $input to $store. toJsAny(input)=${toJsAny(input)}")
        store.add(toJsAny(input))
      }
      def put(input: V, store: IDBObjectStore): IDBRequest = {
        println(s"====> puting $input to $store. toJsAny(input)=${toJsAny(input)}")
        store.put(toJsAny(input))
      }
    }
    implicit object explicitly extends StoreKeyPolicy[(K,V)] {
      ...
      def add(input: (K,V), store: IDBObjectStore): IDBRequest = {
        println(s"====> adding $input to $store . toJsAny(input._2)=${toJsAny(input._2)}")
        store.add(
          toJsAny(input._2),
          input._1.asInstanceOf[js.Any]
        )
      }
      def put(input: (K,V), store: IDBObjectStore): IDBRequest = {
        println(s"====> putting $input to $store . toJsAny(input._2)=${toJsAny(input._2)}")
        store.put(
          toJsAny(input._2),
          input._1.asInstanceOf[js.Any]
        )
